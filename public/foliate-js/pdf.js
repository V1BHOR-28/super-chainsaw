...
function createPageSection(pdf, pageNum) {
    ...
    // createDocument must be a real function in scope, NOT an object property.
    // If it's only a sibling property in the return object below, load()'s
    // `await createDocument()` throws ReferenceError because an object literal's
    // own property is not a variable binding visible to its sibling methods.
    // Declared here so both load() and the section object's createDocument
    // property (kept for downstream consumers like text search) share one fn.
