#!/usr/bin/env python3
"""
tts_brain/epub_parser.py — EPUB → TTS-optimized chapters.

Ported verbatim from https://github.com/gfrangiamone/audiobook-maker
(epub_to_tts.py, lines 1-2042) with explicit permission from the author.

This is the "parser brain": spine-based reading order, TOC reconciliation,
multi-chapter single-file splitting, front/back matter filtering, TTS text
cleaning (footnote markers, URLs, abbreviations, roman numerals, punctuation),
and heading-based chapter detection with LIS dynamic programming.

The CLI main() and output formatters (write_single_file, write_chapter_files,
write_batch_script, print_book_info) from the original module are intentionally
NOT ported — the host pipeline (convert_audiobook.py) owns EPUB download and
chapter dispatch.

Public API for the host pipeline:
    parse_epub_for_tts(epub_path) -> (title, author, chapters)
        chapters: list of {"title": str, "text": str, "order": int}

Dependencies: ebooklib, beautifulsoup4, lxml, tts_brain.secure_archive
"""
...
def should_skip_element(tag) -> bool:
...
def extract_text_from_element(element, depth: int = 0) -> list:
    """
...
# import da epub_to_tts (l'inverso creerebbe un import circolare).
#
...
def _build_toc_fragments(book) -> dict:
    """
...
def _split_html_by_headings(html_content: str, toc_titles: list) -> list:
    """
    ...
    def find_match_indices(heading_text: str) -> tuple:
        """
    ...
    def lis_assign(cand_options: list) -> list:
        """
...
def _split_html_by_headings_auto(html_content: str) -> list:
    """
...
# ═══════════════════════════════════════════════════════════════════
# HOST PIPELINE ADAPTER
# ═══════════════════════════════════════════════════════════════════

def parse_epub_for_tts(epub_path: str):
    """
    Parse an EPUB and return chapters in the host pipeline's dict shape.

    Returns: (title: str, author: str, chapters: list[{"title", "text", "order"}])
    Matches the contract of the old convert_audiobook.parse_epub() so the host
    pipeline can swap in this brain with a single import line.
    """
    ...
    chapters = [
        {
            "title": c.title,
            "text": c.text,
            "order": c.index - 1,  # 0-indexed to match host pipeline convention
        }
        for c in info.chapters
    ]
    return info.title, info.author, chapters
