    ...
    if (tool === 'web_search') {
      try {
        ...
        if (results.length > 0) {
          ...
          toolContext = `REAL-TIME WEB SEARCH RESULTS for "${actualContent}" — today is ${dateStr}.${degradationWarning}\n${results.join('\n\n')}\n\n=== GROUNDING RULES (STRICT — VIOLATING THESE IS THE WORST FAILURE MODE) ===\n1. The search results above are your ONLY source of truth for facts, scores, dates, and recent events. Your training data is STALE and often WRONG for current events. If search data contradicts your memory, the SEARCH WINS. Always.\n2. SCORE EXTRACTION: Read the search results carefully and extract the EXACT score. If NPR says "France downs Morocco 2-0", if ESPN says "France 2-0 Morocco", if multiple sources agree on 2-0 — you say 2-0. NOT 2-1, NOT 1-0, NOT "around 2-0". The EXACT score from the search. Quote it verbatim.\n3. YEAR/TOURNAMENT CONFLATION IS FORBIDDEN: The search results are about a SPECIFIC tournament and year (e.g. "2026 FIFA World Cup"). Do NOT mention or reference any OTHER tournament or year (e.g. 2022, 2018, 2014) unless the search results explicitly discuss it. If you're tempted to say "a rematch of their 2022 semifinal" and the search results don't mention 2022 — DON'T. That's your training data bleeding through. The search results are the only context that exists.\n4. If a result shows a FINAL score, the match is OVER. Report the winner and the score — do NOT describe it as upcoming, hypothetical, or "will be".\n5. If results mention today, yesterday, or "recently", treat that as the current event.\n6. Only fall back to your training data if the search results genuinely do not answer the question AND you have no other option. If you do fall back, explicitly say "I couldn't verify this with current data."\n7. If a DEGRADED SEARCH NOTICE is shown above, you MUST explicitly tell the user your web search was limited to live scores only.\n8. Before stating any score, date, or result, VERIFY it appears in the search results above. If it doesn't appear, don't say it.`
        } else {
